...
export function TeamsSkillsPanel(props: { identity: Identity; onClose: () => void }) {
  ...
  const canEdit = data?.can_edit?.teams_skills === true;
  const teamsSkills = (data?.memory && typeof data.memory === "object" ? (data.memory as any).teamsSkills : null) as any;
  ...
  async function refresh() {
    ...
    try {
      ...
      setData(out);
      const ts = (out?.memory && typeof out.memory === "object" ? (out.memory as any).teamsSkills : null) as any;
      const teams = Array.isArray(ts?.teams) ? ts.teams : [];
      const skills = Array.isArray(ts?.skills) ? ts.skills : [];
      setTeamsText(teams.map(String).join(", "));
      setSkillsText(skills.map(String).join(", "));
      setAvailabilityNotes(typeof ts?.availabilityNotes === "string" ? ts.availabilityNotes : "");
    } catch (e: unknown) {
  ...
  async function save() {
    ...
    try {
      const ops = [
        { op: "set", path: "teamsSkills.teams", value: splitCsv(teamsText), visibility: "team" },
        { op: "set", path: "teamsSkills.skills", value: splitCsv(skillsText), visibility: "team" },
        { op: "set", path: "teamsSkills.availabilityNotes", value: availabilityNotes.trim() || null, visibility: "team" },
      ];
  ...
  return (
    <div style={{ height: "100%", background: "white", display: "grid", gridTemplateRows: "auto 1fr", overflow: "hidden" }}>
      <div style={{ padding: "12px 14px", borderBottom: "1px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
        <span style={{ fontWeight: 700, fontSize: 15, color: "#0f172a" }}>Teams & Skills</span>
          ...
          <>
            <section style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 14, padding: 12, display: "grid", gap: 10 }}>
              <label style={{ display: "grid", gap: 6 }}>
                <span style={{ fontSize: 12, fontWeight: 900, color: "#0f172a" }}>Teams</span>
                <input
                  value={teamsText}
                  onChange={(e) => setTeamsText(e.target.value)}
                  placeholder="e.g. Kids, Welcome, Production"
                  disabled={!canEdit}
                  style={{ padding: "8px 10px", borderRadius: 10, border: "1px solid #e2e8f0", background: canEdit ? "white" : "#f1f5f9" }}
                />
              </label>
              <label style={{ display: "grid", gap: 6 }}>
                <span style={{ fontSize: 12, fontWeight: 900, color: "#0f172a" }}>Skills</span>
                <input
                  value={skillsText}
                  onChange={(e) => setSkillsText(e.target.value)}
                  placeholder="e.g. hospitality, teaching, audio"
                  disabled={!canEdit}
                  style={{ padding: "8px 10px", borderRadius: 10, border: "1px solid #e2e8f0", background: canEdit ? "white" : "#f1f5f9" }}
                />
              </label>
              <label style={{ display: "grid", gap: 6 }}>
                <span style={{ fontSize: 12, fontWeight: 900, color: "#0f172a" }}>Availability notes</span>
                <textarea
              ...
              <div style={{ fontSize: 12, color: "#64748b" }}>Stored under `teamsSkills.*`</div>
            </section>
            ...
            <section style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 14, padding: 12, display: "grid", gap: 6 }}>
              ...
              <pre style={{ margin: 0, whiteSpace: "pre-wrap", fontSize: 12, color: "#334155" }}>{JSON.stringify({ teamsSkills: teamsSkills ?? {} }, null, 2)}</pre>
            </section>
          </>
        )}
      </div>
